...
object GooglePlay {
  ...
  case class GetCard(auth: GoogleAuthParams, pkg: Package)
      extends Ops[AppMissed Xor AppCard] // todo remember this will probably not stay as option
  ...
  class Service[F[_]](implicit I: Inject[Ops, F]) {
    ...
    def getCard(auth: GoogleAuthParams, pkg: Package): Free[F, AppMissed Xor AppCard] =

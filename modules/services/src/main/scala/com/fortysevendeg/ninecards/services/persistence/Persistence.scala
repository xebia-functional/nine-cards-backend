...
class Persistence[K](val supportsSelectForUpdate: Boolean = true) {
  ...
  def generateQuery[A](
    ...
    A: Composite[A],
    K: Composite[K]
  ): Query0[K] = Query[A, K](sql).toQuery0(values)
  ...
  def generateUpdateWithGeneratedKeys[A](
    ...
    implicit
    A: Composite[A]
  ): Update0 = Update[A](sql).toUpdate0(values)
  ...
  def fetchList(
    ...
    implicit
    K: Composite[K]
  ): ConnectionIO[List[K]] = Query[HNil, K](sql).toQuery0(HNil).to[List]
  ...
  def fetchOption[A](
    ...
    A: Composite[A],
    K: Composite[K]
  ): ConnectionIO[Option[K]] = Query[A, K](sql).option(values)
  ...
  def fetchUnique[A](
    ...
    A: Composite[A],
    K: Composite[K]
  ): ConnectionIO[K] = Query[A, K](sql).unique(values)
  ...
  def updateMany[F[_], A](
    ...
    A: Composite[A],
    F: Foldable[F]
  ): ConnectionIO[Int] = Update[A](sql).updateMany(values)

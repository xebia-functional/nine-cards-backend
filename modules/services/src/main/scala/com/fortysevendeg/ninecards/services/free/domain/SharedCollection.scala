...
object SharedCollection {
  ...
  object Queries {
    ...
    val getLatestByCategory = "select * from sharedcollections where category=? order by publishedon desc"
    val getTopByCategory = "select * from sharedcollections where category=? order by installations desc"
